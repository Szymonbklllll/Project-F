...
# Deklaracja zmiennej kontrolującej widoczność ukrytych przycisków
# Deklaracja zmiennej kontrolującej widoczność ukrytych przycisków
# Ustawienie domyślne, aby ukryć dodatkowe przyciski na początku
# default show_hidden_buttons = True
...
#     # Przycisk, który pokaże pozostałe przyciski
#      action Show("main_menu") xpos 0.5 ypos 0.5 anchor (0.5, 0.5)

# # Ekran, który będzie zawierał wszystkie przyciski
# screen main_menu():
#     tag menu
#     frame:
#         xalign 0.5
...
#         vbox:
#             textbutton "Start" action Start()  # Akcja do rozpoczęcia gry
#             textbutton "Opcje" action ShowMenu("preferences")  # Pokaż menu opcji
#             textbutton "Wyjście" action Quit()  # Akcja do wyjścia z gry

# # Przypisz ekran startowy jako domyślny ekran
# label start:
#     call screen start_screen



default show_additional_buttons = False
...
screen navigation():
    ...
    vbox:
        ...
        if main_menu:
            ...
            # Przycisk wyświetlający dodatkowe przyciski
            imagebutton auto "gui/FirstButton_%s.png" focus_mask True action ToggleVariable("show_additional_buttons", True)
            
            # Wyświetlanie dodatkowych przycisków, gdy zmienna show_additional_buttons jest True
            ...
            # Przycisk "FirstButton", który jest widoczny od początku
            # Kliknięcie go zmienia wartość show_hidden_buttons i powoduje, że pozostałe przyciski się pojawiają


        #     imagebutton auto "gui/lg_%s.png" focus_mask True action Start()
            # Wyświetl dodatkowe przyciski tylko, gdy show_hidden_buttons jest True
            # if show_hidden_buttons:

            #     # Przycisk "lg" dla opcji "Start"
            #     imagebutton auto "gui/lg_%s.png" focus_mask True action Start() 

            #     # Przycisk "ls" dla opcji "Preferencje"
            #     imagebutton auto "gui/ls_%s.png" focus_mask True action ShowMenu("preferences") xpos 0.2 ypos 0.3

            #     # Przycisk "ld" dla opcji "Wczytaj"
            #     imagebutton auto "gui/ld_%s.png" focus_mask True action ShowMenu("load") xpos 0.2 ypos 0.4

            #     # Przycisk "pd" dla opcji "Wyjście"
            #     imagebutton auto "gui/pd_%s.png" focus_mask True action Quit(confirm=True) xpos 0.2 ypos 0.5
...
# ShowMenuButton.....
        ...
        # # Przyciski, które są widoczne zawsze
        # imagebutton auto "gui/ls_%s.png" focus_mask True action ShowMenu("load")
        # textbutton _("Preferencje") action ShowMenu("preferences")

        # # Ukryte przyciski, które pojawią się po kliknięciu "FirstButton"
        # if show_hidden_buttons:
        #     vbox:
        #         spacing 10  # Ustawienie odstępów między przyciskami

        #         imagebutton auto "gui/lg_%s.png" focus_mask True action Start()
        #         imagebutton auto "gui/ld_%s.png" focus_mask True action ShowMenu("preferences")
        #         imagebutton auto "gui/ls_%s.png" focus_mask True action ShowMenu("load")
        #         imagebutton auto "gui/pd_%s.png" focus_mask True action Quit(confirm=True)
        ...
        # if renpy.variant("pc"):
        #     # Przycisk zamknij (quit) jest zabroniony w iOS, niepotrzebny w Androidzie i przeglądarkach.
        #     textbutton _("Zamknij") action Quit(confirm=not main_menu)


# Style przycisków nawigacyjnych
...
#     font "fonts/Birds of Paradise Ş PERSONAL USE ONLY.ttf"
#     xalign 0.5
...
screen file_slots(title):
    add "images/save-load.png" yalign 1.0 zoom 1.5
    vbox:
        xalign 0.5
        yalign 0.5
        grid 2 2:
            for i in range(4):
                $slot = i+1
                frame:
                    background Frame(Solid("#27f5f563"))
                    button:
                        xalign 0.5
                        yalign 0.5
                        ...
                        frame:
                            background Solid("#3FBDBF")
                            align (0.5, 0.5)
                            textbutton "Delete this save" action FileDelete(slot) xalign 0.5 yalign 0.5
